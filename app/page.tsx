...
export default function Home() {
  ...
  // Get slider range based on current unit
  ...
  const toCelsius = (temp: number): number => {
    return tempUnit === "F" ? fahrenheitToCelsius(temp) : temp;
  };
  ...
  const handleSurpriseMe = async () => {
    // Random temperature range based on current unit
    let randomMin: number;
    let randomMax: number;

    if (tempUnit === "F") {
      randomMin = Math.floor(Math.random() * 50) + 50; // 50-100°F
      randomMax = randomMin + 10;
    } else {
      randomMin = Math.floor(Math.random() * 20) + 10; // 10-30°C
    ...
    // Trigger search with the calculated values (convert to Celsius for API)
    setLoading(true);
    try {
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-gray-700">
                  Temperature Range: {minTemp}°{tempUnit} - {maxTemp}°{tempUnit}
                </label>
                <button
